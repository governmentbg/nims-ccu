...
ALTER TABLE [AuditAscertainments] ADD
    [OrderNum]        INT             NULL;
GO

UPDATE aa
SET aa.[OrderNum] = calc.[NewOrderNum]
FROM [dbo].[AuditAscertainments] aa
JOIN (SELECT [AuditAscertainmentId], ROW_NUMBER() OVER(PARTITION BY [AuditId] ORDER BY [AuditAscertainmentId]) AS [NewOrderNum]
      FROM [dbo].[AuditAscertainments]) calc on aa.[AuditAscertainmentId] = calc.[AuditAscertainmentId]

GO

ALTER TABLE [AuditAscertainments] ALTER COLUMN 
