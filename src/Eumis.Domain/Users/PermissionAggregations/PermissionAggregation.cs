...
namespace Eumis.Domain.Users.PermissionAggregations
{
    ...
    public class PermissionAggregation
    {
        ...
        {
            var permissionAggregationOne = this;

            if (!(obj is PermissionAggregation permissionAggregationTwo))
            {
                return false;
            }

            CommonPermissionAggregationItemComparer commonComparer = new CommonPermissionAggregationItemComparer();

            foreach (CommonPermissionAggregationItem item in permissionAggregationTwo.CommonPermissions)
            {
                if (permissionAggregationOne.CommonPermissions.Contains(item, commonComparer))
                {
                    continue;
                }
                else
            ...
            ProgrammePermissionAggregationItemComparer programmeComparer = new ProgrammePermissionAggregationItemComparer();

            foreach (ProgrammePermissionAggregationItem item in permissionAggregationTwo.ProgrammePermissions)
            {
                if (permissionAggregationOne.ProgrammePermissions.Contains(item, programmeComparer))
                {
                    continue;
                }
                else
        ...
        {
            return this.CommonPermissions.GetHashCode() ^ this.ProgrammePermissions.GetHashCode();
        }
