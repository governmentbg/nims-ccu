...
{
    internal abstract class BooleanConditionalFunction : INCalcExpressionFunction
    {
        ...
        protected void InitFields(string name, string description, List<Type> expressionParameterTypes, FunctionHandler evaluate)
        {
            ...
            this.ReturnType = NCalcType.Boolean;
        ...
        protected List<bool> EvaluateConditionParameterInGroupMode(List<string> groupList, Expression expression, ProcedureProgramme procedureProgrammeContext, NCalcEvaluationContext evaluationContext)
        {
            List<bool> list = new List<bool>();
            foreach (var groupName in groupList)
            {
                var currentGroupPairs = CurrentGroupParameter.GetContextValue(groupName, procedureProgrammeContext, evaluationContext);
                var groupNamePairs = GroupNameParameter.GetContextValue(groupName);

                Expression selectorExpression = new Expression(expression.ParsedExpression)
                {
                    Parameters = new Dictionary<string, object>(StringComparer.OrdinalIgnoreCase),
                };
                ...
                {
                    selectorExpression.Parameters.Add(pair.Key, pair.Value);
                }

                foreach (var pair in groupNamePairs)
                {
                    selectorExpression.Parameters.Add(pair.Key, pair.Value);
                }

                bool result = (bool)this.EvaluateExpressionHandler(selectorExpression, procedureProgrammeContext, evaluationContext);
