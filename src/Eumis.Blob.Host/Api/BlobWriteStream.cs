    ...
    public class BlobWriteStream : Stream
    {
            ...
            int partitionId)
        {
            this.cmdFirstChunk = new SqlCommand(
                @"UPDATE [dbo].[BlobContents] SET [Content] = @firstChunk WHERE [BlobContentId] = @blobContentId AND [PartitionId] = @partitionId",
                connection,
                transaction);
            this.cmdFirstChunk.Parameters.AddWithValue("@blobContentId", blobContentId);
            this.cmdFirstChunk.Parameters.AddWithValue("@partitionId", partitionId);

            this.cmdAppendChunk = new SqlCommand(
                @"UPDATE [dbo].[BlobContents] SET [Content].WRITE(@chunk, NULL, @length) WHERE [BlobContentId] = @blobContentId AND [PartitionId] = @partitionId",
                connection,
                transaction);
            this.cmdAppendChunk.Parameters.AddWithValue("@blobContentId", blobContentId);
            this.cmdAppendChunk.Parameters.AddWithValue("@partitionId", partitionId);
