...
import tabsDirectiveTemplateUrl from './tabsDirective.html';
...
function TabsDirective($state, $stateParams, $exceptionHandler, l10n) {
  return {
    ...
    restrict: 'E',
    replace: true,
    templateUrl: tabsDirectiveTemplateUrl,
    scope: {
      tabList: '&'
    },
    link: function($scope, iElem, iAttrs) {
      ...
      angular.forEach(_.keys(tabsObject), function(tabTitle) {
        ...
        if (_.isString(tab)) {
          ...
          newTab.state = $state.getWrapper(tab)['abstract']
            ? $state.getWrapper(tab).defaultChild
            : $state.getWrapper(tab);
        } else {
      ...
      function selectTab(tabList, tab, stateName, loading) {
        angular.forEach(tabList, function(tab) {
          tab.isActive = false;
          tab.isCurrentState = false;
        });
        tab.loading = loading;
        tab.isActive = true;

        if ($state.href(stateName) === $state.href(tab.name)) {
          tab.isCurrentState = true;
        }
      }
      ...
      function stateMatch(tabStateParams) {
        return _.pairs(tabStateParams).reduce(function(stateMatching, kvp) {
          var param = kvp[0],
            ...
            paramPresent = {}.hasOwnProperty.call($state, param) && $stateParams[param] === value;
          return stateMatching && (paramUndefined || paramPresent);
        }, true);
      }

      function activateTab(tabName) {
        var tab, childTab, i, j;

        for (i = 0; i < $scope.tabList.length; i++) {
          tab = $scope.tabList[i];

          if (tab.isState) {
            if (!_(tabName).includes(tab.name)) {
              continue;
            }

            if ({}.hasOwnProperty.call(tab, 'stateParams') && !stateMatch(tab.stateParams)) {
              continue;
            }

            selectTab($scope.tabList, tab, tabName, false);

            $scope.secondTabList = [];
            return;
          } else {
            for (j = 0; j < tab.children.length; j++) {
              childTab = tab.children[j];

              if (_(tabName).includes(childTab.name)) {
                selectTab($scope.tabList, tab, tabName, false);
                $scope.secondTabList = tab.children;
                selectTab($scope.secondTabList, childTab, tabName, false);
                return;
              }
            }
          }
      ...
      function stopLoader(tabName) {
        ...
        for (i = 0; i < $scope.tabList.length; i++) {
          ...
          if (tab.isState && _(tabName).includes(tab.name)) {
            if ({}.hasOwnProperty.call(tab, 'stateParams')) {
        ...
        for (j = 0; j < $scope.secondTabList.length; j++) {
      ...
      $scope.openTab = function(newSection) {
        ...
        if ($state.href(newSection.name) === $state.href($state.$current.name)) {
          loading = false;
          return;
        }
        ...
        if (newSection.isState) {
          if (!!newSection.stateParams) {
            ...
            selectTab($scope.tabList, newSection, newSection.name, true);
            $state.go(newSection.state, newStateParams)['catch'](function(error) {
              $exceptionHandler(error);
            });
          } else {
            if (newSection.parent) {
              selectTab($scope.secondTabList, newSection, newSection.name, true);
          ...
          selectTab($scope.tabList, newSection, newSection.name, false);
          $scope.secondTabList = newSection.children;
          selectTab($scope.secondTabList, newSection.children[0], newSection.name, true);
...
export { TabsDirective as scTabsDirective };
