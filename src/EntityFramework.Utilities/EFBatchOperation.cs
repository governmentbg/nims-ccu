#pragma warning disable CS0693 // Type parameter 'TContext' has the same name as the type parameter from outer type 'EFBatchOperation<TContext, T>
...
namespace EntityFramework.Utilities
{
    ...
    public interface IEFBatchOperationBase<TContext, T> where T : class
    {
        ...
        void InsertAll<TEntity>(IEnumerable<TEntity> items, DbConnection connection = null, int? batchSize = null, DbTransaction transaction = null, string tableNameOverride = null) where TEntity : class, T; 
        ...
        where TContext : DbContext
    {
        ...
        /// <param name="connection">The DbConnection to use for the insert. Only needed when for example a profiler wraps the connection. Then you need to provide a connection of the type the provider use.</param>
        /// <param name="batchSize">The size of each batch. Default depends on the provider. SqlProvider uses 15000 as default</param>
        public void InsertAll<TEntity>(IEnumerable<TEntity> items, DbConnection connection = null, int? batchSize = null, DbTransaction transaction = null, string tableNameOverride = null) where TEntity : class, T
        {
            ...
            if (provider != null && provider.CanInsert)
            {
                ...
                if (!mapping.TypeMappings.TryGetValue(entityType, out typeMapping))
                {
                    if (!mapping.TypeMappings.TryGetValue(entityType.BaseType, out typeMapping))
                    {
                        throw new InvalidOperationException(string.Format("No mapping found for type {0}", entityType.ToString()));
                    }
                }
                ...
                provider.InsertItems(items, tableMapping.Schema, string.IsNullOrEmpty(tableNameOverride) ? tableMapping.TableName : tableNameOverride, properties, connectionToUse, batchSize, transaction);
            }
            else
            {
