...
namespace Eumis.Public.Common.Helpers
{
    public static class DataUtils
    {
        ...
        public static int GetMonthsDuration(DateTime d1, DateTime d2)
        {
            var d1Y = d1.Year;
            var d2Y = d2.Year;
            var d1M = d1.Month;
        ...
        public static bool TryParseCalendarDate(string keyword, out DateTime dateTime)
        {
            string format1 = Configuration.CALENDAR_FORMAT_MONTH;
            string format2 = Configuration.CALENDAR_FORMAT_DATE;
        ...
        public static int? LenNoWhiteSpace<T>(T? obj)
            where T : struct
        {
            if (obj != null)
            {
        ...
        public static object Pow(object left, object right)
        {
            double leftDouble;
            double rightDouble;
            if (TryConvert(left, out leftDouble) && TryConvert(right, out rightDouble))
            {
        ...
        public static decimal Percent(decimal value, decimal percentage)
        {
            ...
            return (decimal)((value / 100M) * percentage);
        }

        public static decimal Percent(decimal value)
        {
        ...
        {
            return !Between(valueObject, lowValue, highValue);
        }
        ...
        public static string IntegerToString(int result)
        {
        ...
        public static string IntegerToString(long result)
        {
            NumberFormatInfo nfi = new NumberFormatInfo { NumberGroupSeparator = " ", NumberDecimalDigits = 0 };
        ...
        public static string DecimalToStringWithoutDecimalPointSpace(decimal result)
        {
            NumberFormatInfo nfi = new CultureInfo("en-US", true).NumberFormat;
            nfi.NumberGroupSeparator = " ";
            nfi.NumberDecimalDigits = 0;

            return Math.Round(result).ToString("N", nfi);
        }
        ...
        public static string ToAbsolute(this Uri uri)
        {
            var url = HttpContext.Current.Request.Url;
            var port = url.Port != 80 ? (":" + url.Port) : string.Empty;

            var stringBaseUri = string.Format(
                "{0}://{1}{2}",
                url.Scheme,
                url.Host,
                port);
